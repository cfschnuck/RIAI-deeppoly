...
class DeepPoly():
    ...
    def _build_network_transformer(self):
        ...
        layers +=[DeepPolyPairwiseDifference(self.true_label, last=last, back_sub_steps=self.back_sub_steps, only_first=self.only_first)]
...
class DeepPolyNormalizingTransformer(nn.Module):
    ...
    def forward(self, bounds):
        self.bounds = bounds
        self.bounds = torch.div(bounds - self.mean, self.sigma)
        return self.bounds
